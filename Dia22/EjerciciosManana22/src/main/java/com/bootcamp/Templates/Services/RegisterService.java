...
public class RegisterService {
	...
	public boolean save(int codeSubject, int studentDni) {
		if(!studentRepository.existsByDni(studentDni) || !subjectRepository.existsByCode(codeSubject))
			return false;
		Students student = studentRepository.findByDni(studentDni);
		Subjects subject = subjectRepository.findByCode(codeSubject);
		if(registerRepository.existsByStudent(student) && registerRepository.existsBySubject(subject))
			return false;
