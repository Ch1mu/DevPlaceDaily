...
import com.example.EjerciciosManana18.Reports.Documents.StudentExcelReport;
import com.example.EjerciciosManana18.Reports.Documents.StudentPdfReport;
...
public class StudentController {
    ...
    public void getPdf(HttpServletResponse response) throws DocumentException, IOException
    {
        ...
        response.setHeader("Content-Disposition", "attachment;filename=Student-ListPDF.pdf");
    ...
    public void getExcel(HttpServletResponse response) throws DocumentException, IOException
    {
        ...
        response.setHeader("Content-Disposition", "attachment;filename=Student-ListExcel.xlsx");
        List<Student> students = ss.findAll();
        StudentExcelReport sER = new StudentExcelReport(students);
        sER.export(response);

    }
    @GetMapping("/pdfOrder")
    public void getPdfOrder(HttpServletResponse response) throws DocumentException, IOException
    {
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment;filename=Student-ListPDF.pdf");

        List<Student> students = ss.orderByLastName();
        StudentPdfReport pdfclass = new StudentPdfReport(students);
        pdfclass.export(response);

    }
    @GetMapping("/excelOrder")
    public void getExcelOrder(HttpServletResponse response) throws DocumentException, IOException
    {
        response.setContentType("application/octet-stream");
        response.setHeader("Content-Disposition", "attachment;filename=Student-ListExcel.xlsx");
        List<Student> students = ss.orderByLastName();
